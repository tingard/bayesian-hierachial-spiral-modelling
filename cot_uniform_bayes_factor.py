...
import pymc3 as pm
import warnings
from super_simple.hierarchial_model import BayesFactorBHSM
...
def generate_sample(N_GALS=None, seed=None):
    ...
    normalization = np.concatenate(
        galaxies_df.drop('pipeline', axis=1).T
        .unstack().dropna().apply(lambda a: a.R).values
    ).std()
    galaxies = pd.Series([
        [
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ntune', default=1500, type=int,
                        help='Number of tuning steps to take')
    parser.add_argument('--ndraws', default=3000, type=int,
    ...
    bhsm = BayesFactorBHSM(galaxies.values)
    ...
    p_cot_uniform = trace['model_index'].mean()
    p_normal = 1 - p_cot_uniform
    print(f'Probability of Cot Uniform Model: {p_cot_uniform:.4f}')
