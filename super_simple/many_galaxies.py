...
with pm.Model() as model:
    ...
    global_pa_sd = pm.HalfCauchy('pa_sd', beta=5, testval=3)

    # intra-galaxy dispersion
    gal_pa_sd = pm.HalfCauchy('gal_pa_sd', beta=5, testval=10)

    # arm offset parameter
    arm_c = pm.Normal('c', mu=0, sigma=20, shape=len(gal_arm_map), testval=0)

    # radial noise (degenerate with error on pitch angle I think...)
    sigma = pm.HalfCauchy('sigma', beta=0.5, testval=0.1)
...
with model:
    ...
    gal_pa_mu_offset = pm.Normal('gal_pa_mu_offset', mu=0, sd=1, shape=N_GALS)
    ...
    pm.Potential(
        ...
        (
            ...
            + tt.switch(tt.all(gal_pa_mu < 60), 0, -np.inf)
        )
    )
...
with model:
    print('Test log likelihood: {:.4e}'.format(model.logp(model.test_point)))
